...
def c_pos(subtype, chromosome, suffix = ""):
...
@ray.remote
def get_count_table_control(chromosome, subtype, offset, suffix = ""):
  """
  ...
  control_pos = c_pos(subtype, chromosome, suffix = suffix)
  rev_control_pos = c_pos(subtype + "_rev", chromosome, suffix = suffix)
...
def get_count_all(subtype, offset, status = "singleton", suffix = ""):
  """
  ...
  if status == "singleton":
    ...
    futures = [get_count_table_control.remote(i, subtype, offset, suffix = suffix) for i in range(1,23)]
    results = pd.DataFrame.from_dict(ray.get(futures)).sum(axis=0)
  return(results)

def fit_model_all(subtype, offset, suffix = ""):
  s_tab = get_count_all(subtype, offset, status = "singleton", suffix = suffix)
  s_tab = s_tab.reset_index(level=0)
  s_tab.columns = ['nuc', 'singletons']
  c_tab = get_count_all(subtype, offset, status = "control", suffix = suffix)
...
for offset in range(1, 501):
  print(offset)
  results.append(fit_model_all(subtype, offset * -1, suffix = suffix))
  if subtype.startswith("cpg") and offset == 1:
    continue
  results.append(fit_model_all(subtype, offset, suffix = suffix))

ray.shutdown()
  
final = pd.DataFrame.from_dict(results)
out_dir = "/net/snowwhite/home/beckandy/research/1000G_NYGC_LSCI/output/common/single_pos/"
